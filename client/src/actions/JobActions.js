...
export const createJobPost = (jobData) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post("/api/v1/create/job",jobData,config) ;        
...
export const saveJob = (id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get(`/api/v1/saveJob/${id}`,config) ;
...
export const getSavedJobs = () => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get("/api/v1/getSavedJobs",config) ;
