...
export const createApplication = (id) => async (dispatch) =>{
    try{
        ...
        const { data } = await axios.post(`/api/v1/createApplication/${id}`,config,config) ;
...
export const getAppliedJob = () => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get("/api/v1/getAllApplication",config) ;
...
export const getSingleApplication = (id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get(`/api/v1/singleApplication/${id}`,config) ;
...
export const deleteApplication = (id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.delete(`/api/v1/deleteApplication/${id}`,config)
